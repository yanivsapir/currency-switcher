package test.java.gui;

import main.java.il.ac.shenkar.scaleCurrencies.gui.Gui;
import main.scala.il.ac.shenkar.scaleCurrencies.model.Coin;
import main.scala.il.ac.shenkar.scaleCurrencies.utills.DataAdaptor;
import org.junit.Assert;
import org.junit.Test;

import java.util.HashMap;

/**
 * Created by Golan on 19/07/2015.
 */
public class GuiTest {
    private Coin c1 = new Coin("peso","1","PZO","Argentina","1,2","0.01");
    private Coin c2 = new Coin("dinar","1000","DNR","Turkey","1","0.23");
    private Coin c3=new Coin("sheqel","1","NIS","Israel","1","0");
    private HashMap<String,Coin> map = new HashMap<String,Coin>();
    private DataAdaptor dataAdaptor=new DataAdaptor();
    private String[] countries=dataAdaptor.getCountriesFromFile(map);
    private String[][] data=dataAdaptor.getCoinsDataFromFile(map);
    Gui gui=new Gui(map,data,countries);
    @Test
    public void checkGuiCtor(){
        map.put(c1.getCountry(),c1);
        map.put(c2.getCountry(),c2);
        map.put(c3.getCountry(), c3);
        Assert.assertNotNull(gui);
    }
}
